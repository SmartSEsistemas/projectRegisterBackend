...
import { RolePermissionDTO } from "../dto/permission/RolePermissionDTO";
...
class PermissionServices {
  ...
  async create(body: PermissionDTO) {
    if (await prismaInstance.prisma().register_permission.findFirst({ where: { name: body.name } })) throw new AppError("Permission já cadastrar com o mesmo nome.");
  ...
  async createRolePermission({ permission_id, role_id }: RolePermissionDTO) {
    try {
      ...
      if (!role || !permission) throw new Error("Role ou premission não encontrados");
