...
class UserService {
  ...
  async create(role: RoleDTO, user_id: number) {
    try {
      if(await this.findRole(role)) throw new Error("Role já cadastrada.");
    
     await prismaInstance.prisma().register_role.create({
        data: {
          ...role,
          first_user: user_id
        }
      }).catch(()=> {throw new Error("Error ao cadastrar Role.")});
  ...
  async update(role: RoleDTO, user_id: number) {
    try {
      ...
      await prismaInstance.prisma().register_role.update({
      ...
      }).catch(()=> {throw new AppMessage("Error ao atualizar Role.")})
  ...
  async get(role_id: number) {
    return await prismaInstance.prisma().register_role.findUnique({
      where: { id: role_id} })
      .catch(()=> {throw new AppMessage("Error ao pegar Role.")})
  }
  ...
  async delete(role_id: number): Promise<void> {
    try {
      ...
      await prismaInstance.prisma().register_role.delete({
        where: {id: role_id}
      }).catch(() => {throw new Error('Error ao deletar Role.')});
  ...
  private async findRole({name, type} : RoleDTO) {
    ...
    }).catch(()=> {throw new Error("Error ao procurar Role.")});
  }

  private async checkRole({name, type}: RoleDTO) {
    const role = await prismaInstance.prisma().register_role.findUnique({
      where: {name_type: {name, type}},
      select: {
        Register_roles_permission: true,
        Register_user_role: true,
      }
    }).catch(() => {throw new Error('Error ao pegar Role para deletar/editar.')});

    if(!role) throw new Error('Role não encontrada')
    if(role.Register_roles_permission.length > 0 || role.Register_user_role.length > 0) throw new Error("Role não pode ser deletada/editada pois já foi referenciada.");
 }
}
