...
class LoginService {
  async token(login: LoginDTO): Promise<TokenDTO> {
    ...
    if (!user) throw new AppMessage('Usuário não encontrado ou não cadastrado para essa entidade.', 404);

    if (!user.active) throw new AppMessage('Usuário inativo.', 401);
  ...
  private async findEntity(nameEntity: string) {
    ...
    if (!legalPerson) throw new AppMessage('Pessoa jurídica não encontrada.', 404);

    if (legalPerson.Register_entity.length < 0) throw new AppMessage('Pessoa jurídica sem entidade.', 400);

    return legalPerson.Register_entity[0].id;
  }

  private async findUser(document: string, register_entity_id: number): Promise<Register_user | null> {
  ...
  private comparePassword(password: string, hashPassword: string) {
    ...
    if (!userValid) throw new AppMessage('Nº do documento e/ou senha esta(ão) incorreto(s).', 401);
  }
}
