...
class LoginService {
  async token(login: LoginDTO): Promise<TokenDTO> {
    ...
    if (!user) throw new AppMessage("Usuário não encontrado ou não cadastrado.")
  ...
  private async findEntity(nameEntity: string) {
    const legalPerson = await prismaInstance.prisma().register_legal_person.findFirst({
      where: {corporate_name: nameEntity},
      select: {
        Register_entity: true
      }
    }).catch(() => {throw new AppMessage("Erro ao procurar entidade.")})

    if(!legalPerson) throw new AppMessage("Pessoa jurídica não encontrada.");

    if(legalPerson.Register_entity.length < 0) throw new AppMessage("Pessoa jurídica sem entidade.");
  ...
  private async findUser(document: string, register_entity_id: number): Promise<Register_user | null> {
    ...
    .catch(() => {throw new AppMessage("Erro ao encontrar usuário.")})
  }
