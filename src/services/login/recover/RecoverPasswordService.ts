...
class RecoverServiceService {
  ...
  async resetPassword({document, email, entity_id} : RecoverDTO): Promise<void> {
    
    try {
      const user = await this.findUser(document, entity_id);
      if (!user || !user.active ) throw new Error("Usuário não cadastrado ou inativo.");
      if(!await this.findPerson(user.document, user.type_person, entity_id)) throw new Error("E-mail não cadastrado.");
      ...
      }).catch(() => {throw new Error("Erro ao enviar e-mail para recuperar senha.")})
  ...
  private async findUser(document: string,register_entity_id: number) {
    ...
    }).catch(() => { throw new AppMessage('Usuário não encontrado.') })
  }

  private async findPerson(document: string, typePerson: string, entity_id: number) {
    if(typePerson === "NATURAL") return await prismaInstance.prisma().register_natural_person.findUnique({
      where: {cpf_register_entity_id: {cpf: document, register_entity_id: entity_id}}
    })
    else if(typePerson === "LEGAL") return await prismaInstance.prisma().register_legal_person.findUnique({
