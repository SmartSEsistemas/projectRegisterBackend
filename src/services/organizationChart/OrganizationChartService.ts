...
class OrganizationChartService {
  async create(organizationChart: OrganizationChartDTO, user_id: number) {
    try {
  ...
  async update({organization_chart, reason}: OrganizationChartUpdateDTO, user_id: number, organizationChart_id: number) {
    try {
  ...
  async get(organizationChart_id: number) {
  ...
  async delete(organizationChart_id: number): Promise<void> {
    try {
      await this.checkOrganizationChart(organizationChart_id);
  ...
  private async checkOrganizationChart(id: number) {
    ...
    if(!organizationChart) throw new Error('Organograma não encontrada');
    if(organizationChart.Register_organization_chart_config ) throw new Error("Organograma não pode ser deletada/editada pois já foi referenciada.");
  }

  private async checkIds({register_admin_type_id, register_background_type_id, register_entity_id, register_organization_chart_config_id}: OrganizationChartDTO) {
