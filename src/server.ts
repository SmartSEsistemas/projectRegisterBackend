...
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  if (database === undefined) throw new AppMessage('Nome do banco de dados não enviado')
  
  prismaInstance.setDB(database)
  next()
})
...
app.use((err: Error, request: Request, response: Response, next: NextFunction) => {
  ...
  if (err instanceof ZodError) {
    const messages: string | string[] = JSON.parse(err.message).map((error: any) => `${error.path[0]}: ${error.message}`)
    return response.status(400).json(new AppMessage(messages))
  };

  return response.status(500).json({ Status_code: 500, Message: `Internal server error - ${err.message}` })
})
